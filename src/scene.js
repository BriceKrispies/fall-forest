...
const PATH_COLOR = [0.55, 0.4, 0.25];
const DIRT_COLOR = [0.45, 0.35, 0.22];
const GROUND_GREEN = [0.22, 0.32, 0.14];
const GROUND_MOSS = [0.2, 0.28, 0.12];
...
export function buildScene() {
  ...
  for (let gz = -4; gz < 46; gz += 3) {
    for (let gx = -12; gx < 12; gx += 3) {
      const c = (gx + gz) % 2 === 0 ? GROUND_GREEN : GROUND_MOSS;
  ...
  for (let i = 0; i < PATH_NODES.length - 1; i++) {
    ...
    for (let s = 0; s < steps; s++) {
      ...
      const c = s % 2 === 0 ? PATH_COLOR : DIRT_COLOR;
  ...
  for (const [type, x, y, z, s, r] of trees) {
    ...
    tris.push(...makeShadowPatch(x, gy, z, 0.8 * s));
  }
